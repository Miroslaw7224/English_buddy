...
export async function GET(request: NextRequest) {
  try {
    ...
    const files = fs.readdirSync(levelPath).filter(file => file.endsWith('.json'));
    ...
    for (const file of files) {
      ...
      if (Array.isArray(data)) {
        ...
        // Transform A1-01-greetings.json format to expected format
        cards = data.cards.map((card: any) => ({
          ...
          category: card.topic || card.category || null,
